...
def init_db():
    ...
    cursor = db.cursor()
    cursor.execute("CREATE TABLE taxons(taxon_id INTEGER PRIMARY KEY AUTOINCREMENT, taxon TEXT, ref_len INTEGER, CONSTRAINT taxon_unique UNIQUE(taxon));")
    cursor.execute("CREATE TABLE alignments(alignment_id INTEGER PRIMARY KEY AUTOINCREMENT, taxon_id INTEGER, ref_start INTEGER, ref_end INTEGER, \
...
def populate_db(args):
    ...
    for taxon, hit, read_qual in hits:
        ...
        cigar = hit.cigar_str
        cursor.execute(f"INSERT OR IGNORE INTO taxons(taxon, ref_len) VALUES('{taxon}', '{ref_len}');")
        cursor.execute(f"SELECT * FROM taxons WHERE taxon='{taxon}';")
        taxon_id = cursor.fetchone()["taxon_id"]
        cursor.execute(f"INSERT INTO alignments(taxon_id, ref_start, ref_end, query_start, query_end, strand, cigar, mapping_qual) \
...
def get_strand_bias_metric(alignments):
    ...
    return 1 - abs(sum(strands) / len(strands)) #Get the distance of the bias from 0 -> ideally you have an even number of forward and reverse alignments

def get_high_quality_hits_proportion(alignments, quality_threshold):
    high_quality_hits = [alignment for alignment in alignments if alignment["mapping_qual"] >= quality_threshold]
...
def generate_taxon_metrics(cursor, taxons_with_alignments):
    taxon_metrics = []
    weights = {"normalised_mean_map_qual":0.5, "strand_bias":0.25, "high_quality_proportion":1}
    for taxon in taxons_with_alignments:
        cursor.execute(f"SELECT * FROM taxons INNER JOIN alignments ON taxons.taxon_id = alignments.taxon_id WHERE alignments.taxon_id='{taxon['taxon_id']}';")
        ...
        high_quality_proportion = get_high_quality_hits_proportion(alignment_matches, 50)
        taxon_metric_dict = {"taxon":taxon['taxon'], "n_hits":n_hits, "mean_map_qual":mean_map_qual, "normalised_mean_map_qual":normalised_mean_map_qual, "strand_bias":strand_bias, "high_quality_proportion":high_quality_proportion}
        taxon_metric_dict["probability"] = metrics_to_probability(taxon_metric_dict, weights)
